...
import {
  ...
  registerPublicView,
} from "@/lib/repositories/public-projects";
import { poweredBy, resolveBrand } from "@/lib/branding";
...
interface PageProps {
  params: Promise<{ orgSlug: string; projectSlug: string; token: string }>;
}
...
export async function generateMetadata({
  ...
  const project = await getPublicProjectByToken(orgSlug, token);
...
export default async function ProjectPage({ params }: PageProps) {
  ...
  const [project, organization] = await Promise.all([
    getPublicProjectByToken(orgSlug, token),
    getPublicOrganizationBySlug(orgSlug),
  ]);

  if (!project || !organization) notFound();

  await registerPublicView(token);

  const brand = resolveBrand(organization);
  const partner = brand.showPoweredBy ? poweredBy() : null;
  ...
  return (
    <main
      className="bg-brand-cream text-brand-charcoal"
      id="top"
      style={brand.style}
    >
      ...
      <footer className="border-t border-brand-greige/40 px-[7vw] py-14 text-center">
        {brand.logoUrl ? (
          ...
          <img
            ...
            className="mx-auto max-h-[52px] w-auto object-contain"
          />
        ) : (
        ...
        {partner && (
          <p className="mt-6 text-[9px] uppercase tracking-[0.18em] text-brand-sage/70">
            powered by{" "}
            {partner.url ? (
              <a
                href={partner.url}
                target="_blank"
                rel="noopener noreferrer"
                className="text-brand-sage underline-offset-2 hover:underline"
              >
                {partner.label}
              </a>
            ) : (
